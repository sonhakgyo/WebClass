    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState == XMLHttpRequest.DONE && xhr.status == 200) {
            ...
            if (xhr.responseText.trim() == 'success') {
                document.getElementById('idchecktext').innerHTML = "<span style='color:deepskyblue;size:12px;'>사용할 수 있는 아이디입니다</span>";
                ischeckid.value = 'y';
            } else {
                document.getElementById('idchecktext').innerHTML = "<span style='color:deeppink;size:12px;'>사용할 수 없는 아이디입니다</span>";
            }
        }
    }

    xhr.open("GET", "idcheck_ok.php?userid=" + userid.value, true);
    xhr.send();
}

const idchange = function () {
    const ischeckid = document.getElementById('ischeckid');
    ischeckid.value = 'n';
